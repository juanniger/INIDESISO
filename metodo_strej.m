function ft=metodo_strej(datos_id,metodo)
u=datos_id.u;
y=datos_id.y;
...
tu=cont_recta(1,1)*t_muestreo;
ta=cont_recta(1,2)*t_muestreo;
...
tao=ta*(n-1)^(n-1)/factorial(n-1)*exp(-n+1);

load tabla_strejc

trt=tabla_Strejc(n,3)*tao;
tm=tu-trt;

s=tf('s');
ft=A/(tao*s+1)^n*exp(-tm*s);
